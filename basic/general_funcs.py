...
def unique_vals(object):
    for val in object:
        if object.count(val) > 1:
            first_i = object.index(val)
            while True:
                try:
                    del object[object.index(val)]
                except ValueError:
                    object.insert(first_i, val)
                    break
    return object
...
def normalize_list(list):
    ...
    for i, val in enumerate(list):
        if maximum - minimum:
            list[i] = (val - minimum) / (maximum - minimum)
        else:
            list[i] = val - minimum
    return list
